...
async function loadStateFromSupabase() {
  ...
  try {
    ...
    if (record?.data?.pages) {
      applyRemoteWorkspaceRecord(record, {
        statusMessage: `Cloud synced: ${formatUpdatedAt(record.updated_at)}`,
      });
      return;
    }
...
function subscribeToSupabaseWorkspace() {
  if (!supabaseClient || !supabaseConfig) return;

  if (supabaseChannel) {
    void supabaseClient.removeChannel(supabaseChannel);
  ...
  supabaseChannel = supabaseClient
    ...
    .on(
      ...
      (payload) => {
        ...
        if (record.updated_at && record.updated_at === lastPushedAt) {
          lastAppliedRemoteAt = record.updated_at;
          setSyncStatus(`Cloud live: ${formatUpdatedAt(record.updated_at)}`);
          renderCloudStatus();
          return;
        }

        applyRemoteWorkspaceRecord(record, {
          statusMessage: `Cloud live: ${formatUpdatedAt(record.updated_at)}`,
        });
      }
    )
    .subscribe((status) => {
...
function applyRemoteWorkspaceRecord(record, { statusMessage } = {}) {
  if (!record?.data?.pages) {
    return;
  }

  if (record.updated_at && record.updated_at === lastAppliedRemoteAt) {
    return;
  }
  ...
  remoteReady = true;
  lastAppliedRemoteAt = record.updated_at ?? "";
  setSyncStatus(statusMessage || `Cloud synced: ${formatUpdatedAt(record.updated_at)}`);
  applyAdminState();
  render();
}
